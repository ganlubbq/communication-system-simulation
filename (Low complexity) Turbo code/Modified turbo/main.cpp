/* This is a turbo code simulattion which uses viterbi algorithm only to do the turbo decoding
 * The performance will indeed improve through iteration
 * But it will converge quickly (3 iterations), and the final performance doesn't seem to be attracting
 * Code rate = 1/3
...
int main(){
    ...
    for (i=0;i<points && ber[i-1]>1e-5 ;i++){//simulate till BER ~= 1e-5   
        ...
        //for (t=0;t<repetition;t++){  
            ...
            output(v,snr[i],snr_db[i],y);//generate output under a SNR

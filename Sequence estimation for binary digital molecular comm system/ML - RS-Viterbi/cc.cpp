...
inline float distance(float Z, int state, int inputBit, int _m, int *survivor)
{
   int survivorLen = survivor[0];
   float mean = inputBit*a[0];
...
{  
   ...
   for (int iBit=1;iBit<=(FL);iBit++)
   {
      for(int jState=0;jState<_nState;jState+=2)
      {
         ...
         // Survivor sequence. 
         int survivorLen = min(N-_m,iBit-1);
         int *survivorA = new int [survivorLen +1];
         int *survivorB = new int [survivorLen +1];
         survivorA[0] = survivorLen; survivorB[0] = survivorLen;
         struct TrellisNode *tempA = &_trellisNode[iBit-1][currentStateA];
         struct TrellisNode *tempB = &_trellisNode[iBit-1][currentStateB];
         for (int i=0;i<survivorLen;i++) 
         {
            survivorA[i+1] = (tempA->stateNum)&1;
            survivorB[i+1] = (tempB->stateNum)&1;
            tempA = tempA->pPreviousState;
            tempB = tempB->pPreviousState;
         }
         
         // Path costs of the current state no.1
         float pathMetricA0 = distance(LLR[iBit], currentStateA, 0, _m, survivorA);
         float pathMetricA1 = distance(LLR[iBit], currentStateA, 1, _m, survivorA);
                
         // Path costs of the current state no.2
         float pathMetricB0 = distance(LLR[iBit], currentStateB, 0, _m, survivorB);
         float pathMetricB1 = distance(LLR[iBit], currentStateB, 1, _m, survivorB);

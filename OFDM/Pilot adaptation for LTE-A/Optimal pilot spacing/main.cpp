...
//            complex<double> *r = Rayleigh(x,N0,&rng);
            ...
            if (_nBitError == 0)
            {
               if (pilotSpacing == Nfft/2+1) // Decoding success even with only 2 pilots
               {
                  cout << "Successful tx at pilot spacing = " << pilotSpacing-1 << endl;               
                  totalNumSubcarrierUsed += _preNumSubcarUsed;
                  delete [] y;
                  break;
               }
               else
            ...
            {             
               if (pilotSpacing == 2)
               {
                  ...
                  nPacError++;
                  extern double codeRate;
                  totalNumSubcarrierUsed += (double)FL/codeRate/bitsPerSym;
               }
               else
               {
                  cout << "Successful tx at pilot spacing = " << pilotSpacing-1 << endl;
            ...
            _preNumSubcarUsed = _numSubcarUsed;
         }

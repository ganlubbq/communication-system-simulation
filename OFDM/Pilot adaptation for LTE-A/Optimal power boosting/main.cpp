...
int main()
{
   ...
   const double MaxPwrBoostLimit = getValue("Maximum allowable power boosting", "config.txt");
   const int NumPwrBoostPoints = (int)getValue("Number of power boosting search points in (0, MaxPwrBoostLimit]", "config.txt");
   ...
   for (int iSNR=0;iSNR<points;iSNR++)
   {
      ...
      for (iPac=0;nPacError<frameError;iPac++)
      {
         ...
         for (int iPwrBoost=0;iPwrBoost<NumPwrBoostPoints;iPwrBoost++)
         {
            // pwr boost searching starts from 1.0 to Max and then 0 to 1
            pwrBoost = 1.0 + iPwrBoost*(MaxPwrBoostLimit/NumPwrBoostPoints);
            if (pwrBoost > MaxPwrBoostLimit) { pwrBoost -= MaxPwrBoostLimit; }

            // pwr boost searching starts from 1.0 to 0 and then Max to 1
//            pwrBoost = 1.0 - iPwrBoost*(MaxPwrBoostLimit/NumPwrBoostPoints);
//            if (pwrBoost <= 0) { pwrBoost += MaxPwrBoostLimit; }


            double _numSubcarUsed = 0;
            
            complex<double> *x = tx(u,E_tot,modFormat,bitsPerSym,pwrBoost,&_numSubcarUsed);

//            complex<double> *r = AWGN(x,N0,&rng);
//            complex<double> *r = Rayleigh(x,N0,&rng);
            complex<double> *r = multipathCh(x,N0,&rng,iPac,nPacError);
            ...
            unsigned int _nBitError = 0;
        
            int iBit = 1;
            while (iBit <= FL) 
            {
               if (u[iBit] != y[iBit]) { _nBitError++; }
               iBit++;
            }
            
            // Exit upon finding a power boost value which yields successful tx
            if (_nBitError == 0)
            {
               if (iPwrBoost>0) { cout << "Successful tx at pwrBoost = " << fixed << pwrBoost << endl; }
               totalNumSubcarrierUsed += _numSubcarUsed;
               
               delete [] y;
               break;   
            }
            // Declare pk error if all possible power boost values fail
            else if (iPwrBoost == NumPwrBoostPoints-1)
            {
               cout << "packet Error = " << nPacError << endl;
               totalNumSubcarrierUsed += _numSubcarUsed;
               nPacError++;
               nBitError += _nBitError;

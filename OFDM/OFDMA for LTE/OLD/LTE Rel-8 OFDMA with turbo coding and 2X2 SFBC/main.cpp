...
int main()
{
   ...
   // Nfft*nSlot*bitsPerSym/codeRate ~= Nfft*14*1/3 and then rate match
   const int FL = 15000;//128*6;
...
//      double totalNumSubcarrierUsed = 0;
      ...
      for (iPac=0;nPacError<frameError;iPac++)
      {
         ...
         for (int iOFDMSym=0;iOFDMSym<14;iOFDMSym++)
         {
            complex<double> *x = tx(u,E_tot,iOFDMSym,&rng);
...
//            complex<double> **r = blockRayleigh(x,N0,2,&rng);
      ...
      ber[iSNR] = nBitError/(double)(totFLSimulated);

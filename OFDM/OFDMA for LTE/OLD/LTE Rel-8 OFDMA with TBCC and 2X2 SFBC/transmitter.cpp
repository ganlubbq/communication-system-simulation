...
complex<double> *tx(int *rpdcch, double E_tot, int OFDMSymIdx, LAB524Rand* rng)
{
...
// Debug
   ...
   complex<double> pilotSym = txAttribute.psk.one_pskMod(2,E_car_RS,pilotBit);
   ...
   for (int iRB=0;iRB<NUM_RB;iRB++)
   {
      for (int iCar_RB=0;iCar_RB<N_SC;iCar_RB++)
      {
         ...
         else if (RE_type == RPDCCH)
         {
            ...
            {
               if (modFormat == PSK)
               { x[iCar+1] = txAttribute.psk.one_pskMod(bitPerSym,E_car_data,v_c+nBitTx_c+1); }
               else if (modFormat == QAM)
               { x[iCar+1] = txAttribute.qam.one_qamMod(bitPerSym,E_car_data,v_c+nBitTx_c+1); }
         ...
         else if (RE_type == PDCCH || RE_type == USER_DATA)
         {
            ...
            if (modFormat == PSK)
            { x[iCar+1] = txAttribute.psk.one_pskMod(bitPerSym,E_car_data,randBits+1); }
            else if (modFormat == QAM)
            { x[iCar+1] = txAttribute.qam.one_qamMod(bitPerSym,E_car_data,randBits+1); }

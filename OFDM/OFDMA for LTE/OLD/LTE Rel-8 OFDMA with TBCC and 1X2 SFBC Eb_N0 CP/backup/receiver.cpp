...
{  
   ...
   if (_N0 != N0) { sd = sqrt(Nfft*N0/2.0); _N0 = N0; }
...
sd = 0;
   ...
   {
      channelGain1 = new complex<double> [Nfft +1];
      channelGain1[0] = Nfft;
   }
   if (channelGain2 == NULL)
   {
      channelGain2 = new complex<double> [Nfft +1];
      channelGain2[0] = Nfft;
   }
   ...
   // Perform FFT to aquire the transmitted signal in freq. domain
   // Assume 1 tx ANT.
   ...
   for (int i=1;i<=NUM_USEDCAR;i++)
   {
      ...
      cout << i << " " <<  channelGain1[i] << endl;getchar();

...
int *TBCC::viterbiDecode(double *LLR)
{
   const int len = (int)LLR[0];
   const int FL = (len/3);
   static int FL_2 = FL<<1;
   
   double *cost = new double [_nState];
   double *temp = new double [_nState];

   double finalMetricForState[_nState];
...
for (int iState=0;iState<_nState +1;iState++)
{
   ...
   for (int iBit=1;iBit<=FL;iBit++)
   {
      for(int jState=0;jState<_nState;jState+=2)
      {
         ...
         double pathMetricA0 = LLR[iBit]*_outputBitEachState[currentStateA][0] + LLR[iBit+FL]*_outputBitEachState[currentStateA][1] + LLR[iBit+FL_2]*_outputBitEachState[currentStateA][2];         
         double pathMetricA1 = LLR[iBit]*_outputBitEachState[currentStateA][3] + LLR[iBit+FL]*_outputBitEachState[currentStateA][4] + LLR[iBit+FL_2]*_outputBitEachState[currentStateA][5];
         // Path costs of the current state no.2
         double pathMetricB0 = LLR[iBit]*_outputBitEachState[currentStateB][0] + LLR[iBit+FL]*_outputBitEachState[currentStateB][1] + LLR[iBit+FL_2]*_outputBitEachState[currentStateB][2];
         double pathMetricB1 = LLR[iBit]*_outputBitEachState[currentStateB][3] + LLR[iBit+FL]*_outputBitEachState[currentStateB][4] + LLR[iBit+FL_2]*_outputBitEachState[currentStateB][5];

...
const float trellisExtension = getValue("Trellis extension", "config.txt");
...
inline void CC::_allocateMemoryForTrellis(int FL)
{
   ...
   {
      _trellisLen = (int)(FL*trellisExtension);
      
      // Allocate
      _trellisNode = new TrellisNode* [_trellisLen+1];
      for (int i=0;i<_trellisLen+1;i++) { _trellisNode[i] = new TrellisNode [_nState]; }
      _previousFL = FL;
      // Memorize the state number (For trace back purpose).
      for (int i=0;i<_trellisLen+1;i++)
      { 
...
//   cost[0] = 65536; // Equal weights
   ...
   // Forward recursion
   int iBit = 1;
   for (int iTrellis=1;iTrellis<=_trellisLen;iTrellis++)
   {
      for(int jState=0;jState<_nState;jState+=2)
      {
         ...
         {
            _trellisNode[iTrellis][nextState0].pPreviousState = &_trellisNode[iTrellis-1][currentStateA];
            temp[nextState0] = cost[currentStateA] + pathMetricA0;
         }
         else
         {
            _trellisNode[iTrellis][nextState0].pPreviousState = &_trellisNode[iTrellis-1][currentStateB];
         ...
         {
            _trellisNode[iTrellis][nextState1].pPreviousState = &_trellisNode[iTrellis-1][currentStateA];
            temp[nextState1] = cost[currentStateA] + pathMetricA1;
         }
         else
         {
            _trellisNode[iTrellis][nextState1].pPreviousState = &_trellisNode[iTrellis-1][currentStateB];
            temp[nextState1] = cost[currentStateB] + pathMetricB1;
         }
      }
      
      // For the last trellis step, find the state with the largest metric
      if (iTrellis ==_trellisLen)
      {
         float tempMetric = 0;
         
         for (int i=0;i<_nState;i++)
         {
            if (temp[i]>tempMetric)
            {
               tempMetric = temp[i];
               traceBackState = i;
            }
      ...
      // This is a trick; trick the path cost.
      iBit++;
      if (iBit > FL) { iBit = 1; }
   }
   
//   cout << temp[traceBackState] << endl;
//   cout << traceBackState << endl;
//   getchar();
   ...
   struct TrellisNode *temp1 = &_trellisNode[_trellisLen][traceBackState];

   // Trace back
   for (int i=_trellisLen;i>=1;i--)
   {
      if (i <= FL) { decodedBits[i] = (temp1->stateNum)>>(_m-1); }

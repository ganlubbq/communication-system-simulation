...
int **CC::listViterbiDecode(double *LLR, int nList)
{
   const int len = (int)LLR[0];
   
   double **currentMetric,**nextMetric;
   currentMetric = new double *[_nState];nextMetric = new double *[_nState];
   for (int i=0;i<_nState;i++)
   { currentMetric[i] = new double [nList];nextMetric[i] = new double [nList]; }
   ...
   for (int iBit=1;iBit<=(FL+_m);iBit++)
   {
      for(int jState=0;jState<_nState;jState++)
      {
         ...
         // Path costs
         double w0 = LLR[2*iBit -1]*b0 + LLR[2*iBit]*b1;
         double w1 = LLR[2*iBit -1]*b2 + LLR[2*iBit]*b3;
   ...
   int **decodedBits = new int* [nList];
   for (int i=0;i<nList;i++)
   { decodedBits[i] = new int [FL +1];decodedBits[i][0] = FL; }
   ...
   for (int i=0;i<nList;i++)
   {
      ...
      for (int iBit=(FL+_m)-1;iBit>=1;iBit--)
      {
         ...
         if (iBit <= FL) { decodedBits[i][iBit] = tracePointer->inputBit[_previousRank]; }

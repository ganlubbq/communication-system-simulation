...
inline int diffBits(int *a, int *b, int len)
{
   int count = 0;
   for (int i=0;i<len;i++)
   {
...
//   srand(time(NULL)%60);
   ...
   for (int iSNR=0;iSNR<SNRpoints;iSNR++)
   {
      ...
      for (iRetran=0;iRetran<nRetran;)
      {
         ...
         double *r = Rayleigh(x,N0,&rng);
         
         double fadingAmpBuff[symLen +1];fadingAmpBuff[0] = symLen;
         // MRC fading amplitude
         for (int l=1;l<=symLen;l++) { r[l] *= fadingAmp[l];fadingAmpBuff[l] = fadingAmp[l]; }
         ...
         // Assign an initial value to start the search
         {
            ...
            while (1)
            {
               ...
               {
                  r2[l] = fadingAmp[l]*fadingAmp[l]*g*x[l] + r[l];
               }
         ...
         /**************Find k********************************************/         
         {
            ...
            double *retranNoise = Rayleigh(r3,N0,&rng);
            ...
            while (1)
            {
               ...
               for (int l=1;l<=symLen;l++)
               {
                  r3[l] = r[l] + fadingAmpBuff[l]*k*(fadingAmpBuff[l]*k*x[l] + retranNoise[l]);
               }

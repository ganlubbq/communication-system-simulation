...
int *viterbiDecode(double *r)
{
   ...
   for (int iBit=1;iBit<=(FL+m);iBit++)
   {
      for(int jState=0;jState<nState;jState++)
      {
         ...
         // Path costs
         double w0 = r[2*iBit -1]*b0 + r[2*iBit]*b1;
         double w1 = r[2*iBit -1]*b2 + r[2*iBit]*b3;
               
//         double w0 = r[2*iBit -1]*b0*fadingAmp[2*iBit -1] + r[2*iBit]*b1*fadingAmp[2*iBit];
//         double w1 = r[2*iBit -1]*b2*fadingAmp[2*iBit -1] + r[2*iBit]*b3*fadingAmp[2*iBit];

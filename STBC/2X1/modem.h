...
class PskModulator
{
   public:
      ...
      float *bpskDemod(complex<float> *modulatedSym, float N0, complex<float> *channelGain);
      
      // Gray mapping.
      complex<float> *pskMod(int *bitSequenceToBeModulated);
      
      // Return the LLR of each data bit.
      float *pskDemod(complex<float> *modulatedSym, float N0, complex<float> *channelGain);
...
class QamModulator
{
   public:
      ...
      // Return the LLR of each data bit.
      float *qamDemod(complex<float> *modulatedSym, float N0, complex<float> *channelGain);

void bit_metric_comput(struct complex *,float,int *,int,float *,float *);
void viterbi(float *,float *);                              //sub function,calculate the weights and optimal path
...
int output(int *v,float snr,int *r_d){
   ...
   /*-----------------decoding--------------------*/ 
    ...
    for(i=0;i<iteration;i++){
        ...
        bit_metric_comput(r,N0,id,i,bit_metric[0],bit_metric[1]);
        
        deinterleave(bit_metric[0],bit_metric[1]);
        
        viterbi(bit_metric[0],bit_metric[1]);
...
void bit_metric_comput(struct complex *r,float N0,int *id,int pass,float *bit_metric0,float *bit_metric1){
     ...
     for (i=0;i<(k+m)/2;i++){
         ...
         bit_metric0[3*i]=logsum(temp2,temp[3]);
         temp2=logsum(temp[4],temp[5]);
         temp2=logsum(temp2,temp[6]);
         bit_metric1[3*i]=logsum(temp2,temp[7]);
         temp2=logsum(temp[0],temp[1]);
         temp2=logsum(temp2,temp[5]);
         bit_metric0[3*i+1]=logsum(temp2,temp[4]);
         temp2=logsum(temp[2],temp[3]);
         temp2=logsum(temp2,temp[6]);
         bit_metric1[3*i+1]=logsum(temp2,temp[7]);
         temp2=logsum(temp[0],temp[2]);
         temp2=logsum(temp2,temp[4]);
         bit_metric0[3*i+2]=logsum(temp2,temp[6]);
         temp2=logsum(temp[1],temp[3]);
         temp2=logsum(temp2,temp[5]);
         bit_metric1[3*i+2]=logsum(temp2,temp[7]);
         }
         ...
         temp1=4*0+2*b1+b2;
         bit_metric0[3*i]=temp[temp1];
         temp1=4*1+2*b1+b2;
         bit_metric1[3*i]=temp[temp1];
         temp1=4*b0+2*0+b2;
         bit_metric0[3*i+1]=temp[temp1];
         temp1=4*b0+2*1+b2;
         bit_metric1[3*i+1]=temp[temp1];
         temp1=4*b0+2*b1+0;
         bit_metric0[3*i+2]=temp[temp1];
         temp1=4*b0+2*b1+1;
         bit_metric1[3*i+2]=temp[temp1];
...
void viterbi(float *bit_metric0,float *bit_metric1){
     ...
     for (i=0;i<(k+m)/2;i++){
        ...
        metric[0]=bit_metric0[3*i]+bit_metric0[3*i+1]+bit_metric0[3*i+2];
        metric[1]=bit_metric1[3*i]+bit_metric0[3*i+1]+bit_metric0[3*i+2];
        metric[2]=bit_metric0[3*i]+bit_metric1[3*i+1]+bit_metric0[3*i+2];
        metric[3]=bit_metric1[3*i]+bit_metric1[3*i+1]+bit_metric0[3*i+2];
        metric[4]=bit_metric0[3*i]+bit_metric0[3*i+1]+bit_metric1[3*i+2];
        metric[5]=bit_metric1[3*i]+bit_metric0[3*i+1]+bit_metric1[3*i+2];
        metric[6]=bit_metric0[3*i]+bit_metric1[3*i+1]+bit_metric1[3*i+2];
        metric[7]=bit_metric1[3*i]+bit_metric1[3*i+1]+bit_metric1[3*i+2];

...
void bit_metric_comput(struct complex *,double);
void viterbi(void);
...
void bit_metric_comput(struct complex *r,double N0){     //compute the bit based metrics
     int i,j,h,l,x=0;
     double temp[16],ttemp[8],temp2;
     ...
     for (i=0;i<(k+m)/2;i++){
         ...
         for (j=0;j<4;j++){
             for (l=0;l<2;l++){
                     ...
                     temp2=logsum(temp2,ttemp[3]);
                     temp2=logsum(temp2,ttemp[4]);
                     temp2=logsum(temp2,ttemp[5]);
                     temp2=logsum(temp2,ttemp[6]);
                     bit_metric[l][4*i+j]=logsum(temp2,ttemp[7]);
                     }   
             }
         }
     }

     ...
     for (i=0;i<(k+m)*2/3;i++){
         ...
         for (j=0;j<3;j++){
                 for (l=0;l<2;l++){
                     ...
                     temp_llr[l][3*i+j]=max(ttemp);
                     }   
             // Log Likelihood Ratio = logP1 - logP0
             llr[3*i+j]=temp_llr[1][3*i+j]-temp_llr[0][3*i+j];
             }
         }
     ...
     for (i=0;i<k_m;i++){
         ...
         for(j=0;j<st_num;j++){
             ...
             if (cost[j]!=0){
               ...
               w0=(2*out_table[j][0]-1)*llr[2*i]+(2*out_table[j][1]-1)*llr[2*i+1];
               w1=(2*out_table[j][2]-1)*llr[2*i]+(2*out_table[j][3]-1)*llr[2*i+1];

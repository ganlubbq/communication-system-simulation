...
int *turboEncode(int *d)
{
   ...
   {  //encoding
      ...
      for (int j=0;j<m+1;j++)
      {
         t0^=a[i-j+m]*(G12>>(m-j)&1);
         t1^=b[i-j+m]*(G12>>(m-j)&1);
      }
...
int *bcjrDecode(float *r, float N0, int iteration)
{
    ...
    float sd = sqrt(N0/2);//standard deviation of gaussian r.v.
...
void siso(int pass,float *y,int option,float N0,float *LLR,float *z,int k)
{
   ...
   for (int j=0;j<k+m;j++)
   {
      for (int l=0;l<st_num;l++)
      {
         ...
         float term1 = A[j][from_table[l][0]]+pI_sym[j][idxFor0][1];  
         float term2 = A[j][from_table[l][1]]+pI_sym[j][idxFor1][1];
...
//         A[j+1][l]=max(temp,2);
   ...
   for (int j=k+m-1;j>=0;j--)
   {
      for (int l=0;l<st_num;l++)
      {
         ...
         float term1 = B[j+1][next_table[l][0]]+pI_sym[j][idxFor0][1];
         float term2 = B[j+1][next_table[l][1]]+pI_sym[j][idxFor1][1];

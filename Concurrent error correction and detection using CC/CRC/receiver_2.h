...
int output(int *v,float snr,int *y){
    ...
    while (j<(2*(k+m))){
              ...
              rx[j]+=gaurand()*sd;        //add noise
              
              ray[j]=rayrand();
              //rx[j]=gaurand()*sd+r[j]*ray[j];
              j++;
              }
     ...
     for (i=0;i<(k+m);i++){
        ...
        for(j=0;j<st_num;j++){
             ...
             if (cost[j]!=0){
               ...
               w0 = rx[2*i]*b0 + rx[2*i+1]*b1;     //path costs
               w1 = rx[2*i]*b2 + rx[2*i+1]*b3;

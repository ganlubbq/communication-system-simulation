...
int g11[m+1],g12[m+1],gcrc[r+1];                       //generator sequence
...
float ray[2*(k+m)]; //record of the rayleigh r.v.
...
void generator_enumerate(){
     ...
     for (i=0;i<r+1;i++){
         gcrc[i]=temp1/(int)pow(2,r-i);
         temp1=temp1%(int)pow(2,r-i);
         }
     gcrc[0]=1;
     }

     ...
     for (i=0;i<(k+m)/2;i++){
         ...
         if(pass==0){
             ...
             for (j=0;j<8;j++){
                 ...
                 cache[j][i]=2*(mapping[j].re*r[i].re*ray[i] + mapping[j].im*r[i].im*ray[i])/N0;
                 }
             ...
             for (j=0;j<3;j++){
                 for (l=0;l<2;l++){
                     ...
                     temp2=logsum(ttemp[0],ttemp[1]);
                     temp2=logsum(temp2,ttemp[2]);
                     bit_metric[l][3*i+j]=logsum(temp2,ttemp[3])-id[l][3*i+j]*mul;
    ...
    if(pass!=iteration-1){ 
     for(b=0;b<3;b++){
      for (j=0;j<(k+m)/2;j++){ // K
          ...
          /*id[0][3*j+b]=max(t0,32);
          id[1][3*j+b]=max(t1,32);*/       
